...
def init_thread(kern_len, _data, mjds, errs=None):
    ...
    term1 = cel.terms.Matern32Term(np.log(1), np.log(kern_len), bounds=dict(log_sigma=(-15, 15), log_rho=(np.log(82.22), np.log(2540.25))))
...
def lnprob(p):
    ...
    if p[1] < np.log(82.22) or p[1] > np.log(2540.25):
        return(-np.inf)
